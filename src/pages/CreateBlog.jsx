...
export default function CreateBlog() {
  ...
  const calculateReadingTime = (content) => {
    ...
    try {
      ...
      if (parsedContent.root && parsedContent.root.children) {
        const textContent = parsedContent.root.children.map(extractText).join(' ');
    ...
    try {
      ...
      const convertNode = (node) => {
        ...
        if (node.type === 'paragraph') {
          const childText = node.children ? node.children.map(convertNode).join('') : '';
          return childText + '\n\n';
        }
        ...
        if (node.type === 'list') {
          const items = node.children ? node.children.map((item, index) => {
            const itemText = item.children ? item.children.map(convertNode).join('') : '';
      ...
      if (parsedContent.root && parsedContent.root.children) {
        ...
        if (editorElement) {
          ...
          images.forEach(img => {
            const imageMarkdown = `\n![${img.dataset.altText || 'Image'}](${img.dataset.imageUrl})\n`;
  ...
  const uploadImage = async (file) => {
    try {
      ...
      const fileName = `blogs/${Date.now()}-${Math.random().toString(36).substr(2, 8)}.${fileExt}`;
  ...
  const handleSave = async (publishNow = false) => {
    ...
    try {
      ...
      const contentToSave = publishNow ? convertToMarkdown(formData.content) : formData.content;
      ...
      const blogData = {
        ...
        description: formData.excerpt.trim() || contentToSave.substring(0, 200) + (contentToSave.length > 200 ? '...' : ''),
          ...
          <div className="grid grid-cols-12 gap-6 w-full min-h-full">
            ...
            <aside className="col-span-12 lg:col-span-4 xl:col-span-3">
              <div className="h-full overflow-y-auto space-y-6 pb-6">
                ...
                {/* Featured Image Card */}
                <div className="bg-white p-6 rounded-xl border border-gray-200">
                  <h3 className="text-lg font-semibold text-gray-900 mb-5">Featured Image</h3>
                  <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    {featuredImage ? (
                      <div className="w-full">
                        <img
                          src={featuredImage.fromGallery ? featuredImage.url : URL.createObjectURL(featuredImage)}
                          alt="Preview"
                ...
                {/* SEO & Meta Card */}
                <div className="bg-white p-6 rounded-xl border border-gray-200">
                  <h3 className="text-lg font-semibold text-gray-900 mb-5">SEO & Meta</h3>
                  <div className="space-y-4">
                    <div>
