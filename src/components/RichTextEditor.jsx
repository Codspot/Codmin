...
const uploadImageToSupabase = async (file) => {
  try {
    ...
    const fileName = `media/${Date.now()}-${Math.random().toString(36).substr(2, 8)}.${fileExt}`;
...
function ToolbarPlugin({ onFormat }) {
  ...
  return (
    <div className="border-b border-gray-200 px-4 py-3 flex items-center justify-start space-x-1 bg-gray-50">
      ...
      {/* Image Upload */}
      <button
        className={`p-2 text-gray-600 hover:bg-gray-200 hover:text-gray-800 rounded-md transition-colors ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}
    ...
    <div className="bg-white rounded-xl border border-gray-200 h-full flex flex-col min-h-0">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <LexicalComposer initialConfig={initialConfig}>
          <ToolbarPlugin />
          <div 
            className={`relative flex-1 ${isDragOver ? 'bg-blue-50 border-blue-300' : ''} ${isUploading ? 'opacity-75' : ''}`}
            ...
            <RichTextPlugin
              contentEditable={
                <ContentEditable 
                  ...
                  style={{ fontSize: '16px', lineHeight: '1.6', minHeight: '500px' }}
                />
              }
            ...
            {isDragOver && (
              <div className="absolute inset-0 bg-blue-50 bg-opacity-90 flex items-center justify-center border-2 border-dashed border-blue-300 rounded">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm font-medium text-blue-600">Drop images here to upload</p>
                </div>
              </div>
            )}
            ...
            {isUploading && (
              <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm font-medium text-gray-600">Uploading images...</p>
                </div>
              </div>
            )}
          </div>
